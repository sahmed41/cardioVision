...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) { 
    ...
    if ($result_so->num_rows == 0) { // Preventing listing the same physician who the result is already shared with.
      if ($row['id'] != $_SESSION['id']) { // Removing the physician who is sharing the result from the add physician list
      ?> 
